    ...
    <div className="max-w-3xl mx-auto px-4 py-6 flex flex-col gap-8 w-full">
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        {MODES.map((m) => {
          ...
          const content = (
            ...
            >
              <div className="flex items-start justify-between">
                ...
                {!m.live && (
                  <span className="text-[0.6rem] uppercase tracking-[0.25em] text-muted-foreground border border-border/70 rounded-full px-2 py-0.5">
                    Soon
                  </span>
                )}
              </div>
          ...
          return m.live ? (
            <Link key={m.slug} href={`/games/new?mode=${m.slug}`} className="block">
              {content}
            </Link>
          ) : (
