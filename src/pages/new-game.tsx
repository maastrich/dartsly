...
export function NewGameForm({
  ...
  const [outMode, setOutMode] = usePersistentState<X01OutMode>("dartsly.x01.outMode", "single");
  ...
  const [killerLives, setKillerLives] = usePersistentState<3 | 5>("dartsly.killer.startLives", 3);
  ...
  const [randomOrder, setRandomOrder] = usePersistentState<boolean>("dartsly.randomOrder", false);
                ...
                {X01_START_SCORES.map((s) => {
                  ...
                  return (
                    <button
                      ...
                      className={[
                        "h-14 rounded-xl border-2 font-display tabular font-black text-lg transition-all",
                        active
                          ? "bg-[var(--dart-gold)] text-[var(--field)] border-[var(--dart-gold)]"
          ...
          {mode === "cricket" && (
            <section className="flex flex-col gap-4">
                ...
                {CRICKET_SCORING.map((o) => {
                  ...
                  return (
                    <button
                      ...
                      className={[
                        "rounded-xl border-2 px-3 py-3 flex flex-col items-start gap-1 text-left transition-all",
                        active
              ...
              <button
                ...
                className={[
                  "rounded-xl border-2 px-4 py-3 flex items-center justify-between text-left transition-all",
                  cricketCap
                    ? "bg-card border-[var(--dart-gold)]"
                    : "bg-card/60 border-border hover:border-foreground/40",
                ].join(" ")}
                    ...
                    {(
                      [
                        { key: "safe", label: "Safe", hint: "Re-hitting own double is harmless" },
                        {
                          key: "suicide",
                          label: "Suicide",
                          hint: "Own double after armed = lose a life",
                        },
          ...
          <section className="flex flex-col gap-3">
            ...
            <ul className="flex flex-col gap-2">
              {players.map((p) => {
                ...
                const picked = idx >= 0;
                const showKillerPick = picked && mode === "killer" && killerAssignment === "manual";
                return (
                  <li key={p.id} className="flex flex-col gap-1.5">
                    <button
                      ...
                      onClick={() => toggle(p.id)}
                      className={[
                        "w-full flex items-center justify-between rounded-xl border-2 px-4 h-14 text-left transition-all",
                        picked
                          ? "bg-card border-[var(--dart-gold)]"
                          : "bg-card/60 border-border hover:border-foreground/40",
                      ].join(" ")}
                    ...
                    {showKillerPick && (
                      <TargetPicker
                        ...
                        taken={Object.entries(killerTargets)
                          ...
                          .map(([, v]) => v as number)}
                        onChange={(n) => setKillerTargets((prev) => ({ ...prev, [p.id]: n }))}
                      />
                    )}
                  </li>
                );
      ...
      >
        <div className="max-w-3xl mx-auto px-4 pt-3 w-full">
          <button
            ...
            onClick={() =>
              start(async () => {
                ...
                if (mode === "x01") config = { startScore, outMode };
                  ...
                  const targets = buildKillerTargets(ids, killerAssignment, killerTargets);
                  if (!targets) {
            ...
            className="w-full h-14 rounded-xl bg-[var(--dart-gold)] text-[var(--field)] font-display font-extrabold uppercase tracking-[0.3em] text-sm shadow-[0_1px_0_oklch(1_0_0/0.15)_inset,0_8px_24px_-8px_oklch(0.82_0.13_85/0.5)] transition-all active:scale-[0.99] active:translate-y-px disabled:bg-[var(--field-raised)] disabled:text-muted-foreground disabled:shadow-none disabled:cursor-not-allowed"
          >
...
export default function NewGamePage() {
  ...
  const mode: GameMode = modeParam === "cricket" || modeParam === "killer" ? modeParam : "x01";

  const players = useLiveQuery(() => db.players.orderBy("name").toArray(), [], [] as PlayerOpt[]);
